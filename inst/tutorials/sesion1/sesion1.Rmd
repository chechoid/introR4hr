...
`introR4hr` no es un paquete publicado en [CRAN](https://cran.r-project.org/) por lo cual hay que descargar la versión de desarrollo desde el [repositorio de GitHub](https://github.com/chechoid/introR4hr) y seguir los siguientes pasos:

```{r intror4hr1, eval=FALSE}
...
Uno de los paquetes de tidyverse, el paquete `dplyr` aporta un símbolo conocido como *"pipe"* `%>%` (*tubo* en inglés). El atajo de teclado es con las teclas *Ctrl + Mayús + M* en Windows, o *Cmd + Shift + M* en Mac.
...
# Para ver la estructura del data frame usa la función str(nombre_dataframe)
```
...
Un error frecuente que cometemos cuando estamos aprendiendo sobre `tidyverse` es usar la función `filter()` con un sólo signo igual. Probemos correr la siguiente línea de código:
