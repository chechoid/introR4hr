...
![Estados de ánimo de lxs programadores](images/mood_coder.png){width="40%"}
...
```{r objeto-hint-1}
# Asigna el elemento 'bajas' un valor cualquiera. Por ejemplo, las desvinculaciones en tu empresa en los últimos 12 meses 
...
¿Qué tipo de vector genera este código?
...
quiz(
  question("¿Qué tipo de vector genera el código anterior?",
           ...
           answer("Texto", correct = TRUE, message = "Correcto! Cuando a un número lo escribimos entre comillas R lo interpreta como texto"),
...
```{r vec-ej3-hint-1}
# Para elegir un elemento de un vector pone el nombre del vector y entre corchetes el número del elemento
...
Por ejemplo, creemos dos vectores, uno con el nombre de las áreas de `RH`, `Finanzas` y `Operaciones`, y otro con la cantidad de colaboradores, por ejemplo: `6`, `12`, `24` respectivamente. Luego con esos vectores podemos crear un data frame llamado `dotacion`.
...
# El data frame plantel ya está cargado
...
quiz(
  question(
    "¿Cuántas filas y columnas tiene el dataframe 'plantel'?",
  ...
  answer("522 variables y 8 filas", message = "Mirá de nuevo la primera fila del resultado de la estructura del dataframe plantel. Primero indica las observaciones, y luego las variables"),
...
1.  Entrar en pánico 😱
2.  Si de la computadora no sale humo, respirar con alivio hasta recuperar la calma 🧘
3.  Leer el mensaje de error 👩‍💻
4.  En caso de no entender el mensaje, pegar un print de pantalla en el canal **#auxilio** del [grupo de Slack](https://join.slack.com/t/r4hr/shared_invite/zt-ocriyx5e-nqXuwWeDyOKm2ICUJjhu6g) 🙋 que alguien siempre va a ayudar.
...
```{r error2-hint-1}
# Para filtrar por un valor de texto necesitamos dos signos iguales
```
...
Nuevamente, la pista la tenemos detrás de la letra `i`
...
Cuando las opciones son más de dos, la mejor forma de filtrar elementos es usando el operador `%in%` en conjunto con un vector que tenga los valores que nos interesa filtrar. La sintaxis es la siguiente:
...
```{r filters3-hint-1}
# Poner primero la columna que quiero filtrar, usar el operador %in% y luego dentro del vector los nombres de las áreas.
# Los nombres de los sectores están con todas las letras en mayúsculas.
```
